...
class P_Characteristic(Characteristic):
    ...
    def onWriteRequest(self, data, offset, withoutResponse, callback):
        ...
        if p_cmd_value == 0:  # 00：前後両方のサウンドカードをミュート
            call(["sudo", "-u", "pi", "amixer", "-c", str(Front_SC_No),
                 "sset", "Speaker", str(P_00_Front_Vol) + "%", "unmute"])
            call(["sudo", "-u", "pi", "amixer", "-c", str(Rear_SC_No),
                 "sset", "Speaker", str(P_00_Rear_Vol) + "%", "unmute"])
        elif p_cmd_value == 1:  # 01：前方のサウンドカードから出力
            call(["sudo", "-u", "pi", "amixer", "-c", str(Front_SC_No),
                 "sset", "Speaker", str(P_01_Front_Vol) + "%", "unmute"])
            call(["sudo", "-u", "pi", "amixer", "-c", str(Rear_SC_No),
                 "sset", "Speaker", str(P_01_Rear_Vol) + "%", "unmute"])
        elif p_cmd_value == 2:  # 02：後方のサウンドカードから出力
            call(["sudo", "-u", "pi", "amixer", "-c", str(Front_SC_No),
                 "sset", "Speaker", str(P_02_Front_Vol) + "%", "unmute"])
            call(["sudo", "-u", "pi", "amixer", "-c", str(Rear_SC_No),
                 "sset", "Speaker", str(P_02_Rear_Vol) + "%", "unmute"])
        elif p_cmd_value == 3:  # 03：前後両方のサウンドカードから出力
            call(["sudo", "-u", "pi", "amixer", "-c", str(Front_SC_No),
                 "sset", "Speaker", str(P_03_Front_Vol) + "%", "unmute"])
            call(["sudo", "-u", "pi", "amixer", "-c", str(Rear_SC_No),
                 "sset", "Speaker", str(P_03_Rear_Vol) + "%", "unmute"])
        # ----------------------------
